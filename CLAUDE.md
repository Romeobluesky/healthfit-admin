...
| 레벨 | 이름 | 레이블 | 데이터 범위 |
|------|------|--------|------------|
| 10 | SUPER_ADMIN | 최고관리자 | 전체 (삭제 등 전체 권한) |
| 9 | ADMIN | 관리자 | 전체 |
| 8 | PARTNER | 파트너 | 본인 + 하위 협력사 |
| 7 | PARTNERSHIP | 협력사 | 본인만 |

### 권한 체크 유틸리티

`src/lib/permission.ts`에서 권한 판별 함수를 제공한다. (`PERMISSION` 상수는 `src/types/index.ts`)

| 함수 | 조건 | 용도 |
|------|------|------|
| `getPermissionLabel()` | 레벨 → 한글 레이블 | UI 표시 |
| `isAdmin()` | `>= 9` | 관리자/최고관리자 (계층형) |
| `isSuperAdmin()` | `>= 10` | 최고관리자 전용 |
| `isPartner()` | `=== 8` | 파트너 정확히 일치 |
| `isPartnership()` | `=== 7` | 협력사 정확히 일치 |
| `isPartnerOrPartnership()` | `8 \|\| 7` | 파트너 전용 메뉴 표시 대상 |

- 관리자 계열(9/10)은 범위 비교(`>=`), 파트너/협력사(8/7)는 정확 일치(`===`)로 판별한다.
- 권한별 데이터 노출은 API 응답 후 **클라이언트에서 `partnerId` 기준으로 재필터링**한다.
  - 파트너(8): 본인 데이터 + 본인을 상위(`partnerId`)로 둔 협력사 데이터
  - 협력사(7): 본인 데이터만
